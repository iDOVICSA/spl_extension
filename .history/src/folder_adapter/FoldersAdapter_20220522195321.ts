...
export class FoldersAdapter {
    ...
    async adaptFolders(foldersVariants: readonly vscode.WorkspaceFolder[], uris: any, excludeFilter: string[]): Promise<Map<string, string[]>> {
            ...
            if (Utils.ifSelected(folder.uri, uris)) {
                ...
                for (let index = 0; index < result.length; index++) {
                    ...
                    if (filesVariantsMap.get(element)) {
                        ...
                        if (!excludeFilter.includes(element)) {
