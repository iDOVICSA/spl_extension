            ...
            try {
                if (variantsOfBlock.length > 1) {
                    for (let index = 0; index < block.blockContent.get(variantsOfBlock[0])?.length!; index++) {
                        ...
                        console.log(element.element.fileName.path.replace(variantsOfBlock[0], ""));
                        let filePath = resulltPath + path.sep + element.element.fileName.path.replace(variantsOfBlock[0], "");
