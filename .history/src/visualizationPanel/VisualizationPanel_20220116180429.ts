...
export class VisualizationPanel {
	...
	public iterable = new Map([['variant 1', [0, 1, 2, 3, 8]], ['variant 2', [0, 1, 3, 6]], ['variant 3', [1, 3, 4, 5]], ['variant 4', [1]], ['variant 5', [0, 2, 7]]]);
	...
	public showVariants(blocksByVariant: Map<number, number[]>) {
		// Send a message to the webview webview.
		// You can send any JSON serializable data.
		const jsonObject: any = {};
		this.iterable.forEach((value, key) => {
	...
	private _getHtmlForWebview(webview: vscode.Webview) {
		...
		const scriptPathOnDisk = vscode.Uri.joinPath(this._extensionUri, '../../media/webView', 'script.js');

		// And the uri we use to load this script in the webview
		const scriptUri = (scriptPathOnDisk).with({ 'scheme': 'vscode-resource' });

		// Local path to css styles
		const styleResetPath = vscode.Uri.joinPath(this._extensionUri, '../../media/webView', 'main.css');
