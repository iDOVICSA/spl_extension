    ...
    public createFeatureModel() {
        ...
        for (let index = 0; index < this.mutexConstraints.length; index++) {
            ...
            if (altF1 === undefined && altF2 === undefined) {
                ...
                this.noRedundantMutexConstraints.forEach((value, index) => {
                    if (value === element) { this.noRedundantMutexConstraints.splice(index, 1); }
                });
            }
        ...
        for (let index = 0; index < altGroupList.altGroups.length; index++) {
            ...
            let fakeAlternative = new Feature(null, "Alternative_" + element.id, -2 - index, false);
        ...
        this.listOfFeatures.forEach(f => {
            ...
            if (definitiveList.size > 0) {
                ...
                // Get the one with higher number of existense in the requires
                // constraint

                // constraint
                if (parent === undefined) {
                    let maximumReasons = -1;
                    definitiveList.forEach(element => {
                        let reasons = Utils.getNumberOfExistenceOf(f, this.reqConstraints);
                        if (maximumReasons < reasons) {
                            parent = element;
                        }
                    });
                ...
                if (altGroup === undefined) {
                    parent?.children.set(f.featureId, f);
                    f.parent = parent;
                    parentAssigned.push(f);
                } else {
                    if (!parentAssigned.includes(altGroup.altRoot!)) {
                        altGroup.altRoot!.parent = parent;
                        parent?.children.set(altGroup.altRoot!.featureId, altGroup.altRoot!);
                        parentAssigned.push(altGroup.altRoot!);
                    }
                }
        ...
        Utils.deleteRedundantReqConstraint(this.reqConstraints, this.listOfFeatures);
